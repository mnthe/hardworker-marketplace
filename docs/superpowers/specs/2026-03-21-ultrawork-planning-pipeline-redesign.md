...
| # | 결정 | 선택 | 근거 |
|---|------|------|------|
| D1 | Auto-trigger 메커니즘 | keyword-detector.js 확장 + `.claude/rules/` 규칙 추가 | Hook이 이미 UserPromptSubmit에서 동작, 확장 자연스러움 |
| D2 | 감지할 키워드 범위 | 구현 관련 한국어/영어 키워드 ("구현", "만들어", "리팩토링", "implement", "build", "refactor") | 너무 넓으면 false positive, 너무 좁으면 놓침 — 구현 동사에 집중 |
| D3 | Brainstorming과의 공존 | `.claude/rules/`에 라우팅 규칙 추가 — "구현 작업은 ultrawork-plan 사용" | Brainstorming 스킬 자체를 수정할 수 없으므로 규칙으로 우회 |
...
User prompt → keyword-detector (구현 키워드 확장) → /ultrawork-plan 제안
                                                    ↓
              .claude/rules/ (brainstorming 대신 ultrawork 라우팅)
                                                    ↓
...
| 파일 | 변경 유형 | 변경 내용 |
|------|----------|----------|
| `src/hooks/keyword-detector.js` | 수정 | 구현 키워드 패턴 확장, `/ultrawork-plan` 제안 메시지 출력 |
| `.claude/rules/ultrawork-routing.md` | 신규 | "구현 작업은 brainstorming 대신 ultrawork-plan 사용" 라우팅 규칙 |
| `skills/planning/SKILL.md` | 수정 | Phase 4 (Write Design)에서 Execution Strategy 분리, Phase 5 순서 변경 |
| `skills/planning/references/design-template.md` | 수정 | Execution Strategy를 "Post-Review" 섹션으로 이동, 선택적 표시 |
| `agents/planner/AGENT.md` | 수정 | PLANNING tier 워크플로우에서 분리된 순서 반영 |
| `commands/ultrawork-plan.md` | 수정 | Interactive mode에서 분리된 순서 반영 |
...
| `.claude/rules/` | 전체 프로젝트 | 다른 프로젝트에서는 이 규칙 불필요 |
...
2. `.claude/rules/ultrawork-routing.md` — brainstorming → ultrawork 라우팅 규칙
...
|---|-----------|---------|-----------------|
| V1 | keyword-detector.js에 구현 키워드 패턴 존재 | `grep -c "구현\|만들어\|리팩토링\|implement\|refactor" plugins/ultrawork/src/hooks/keyword-detector.js` | 1+ |
| V2 | keyword-detector.js가 advisory mode | `grep -c "제안\|suggest\|advisory" plugins/ultrawork/src/hooks/keyword-detector.js` | 1+ |
| V3 | ultrawork-routing.md 규칙 파일 존재 | `test -f .claude/rules/ultrawork-routing.md && echo "exists"` | exists |
...
| Rules 파일이 brainstorming을 override 못함 | High | 테스트 후 대안 검토 (superpowers 설정 변경 등) |
