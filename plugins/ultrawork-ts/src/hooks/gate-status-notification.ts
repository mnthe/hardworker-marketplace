...
import { getSessionDir, getSessionFile, isSessionActive, updateSession } from '../lib/session-utils';
 ...
 */
function createEmptyResponse(): PostToolUseOutput {
  return {
    hookSpecificOutput: {
      hookEventName: 'PostToolUse'
    }
 ...
 */
function createNotificationResponse(message: string): PostToolUseOutput {
  return {
    hookSpecificOutput: {
      hookEventName: 'PostToolUse',
 ...
 */
function isExplorerTask(hookInput: HookInput): boolean {
...
3. Spawn targeted explorers for each hint`;
      ...
      if (fs.existsSync(contextPath)) {
        ...
        try {
          ...
          context = JSON.parse(contextContent) as Context;
        } catch {
          console.log(JSON.stringify(createEmptyResponse()));
          process.exit(0);
          return;
