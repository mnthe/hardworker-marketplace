...
function buildContextMessage(sessionId, session, taskCount, evidenceCount, sessionFile) {
    const { goal, phase, exploration_stage: explorationStage, options } = session;
    const { auto_mode, skip_verify, plan_only, max_workers } = options;
    const nextAction = buildNextAction(phase, explorationStage, auto_mode, max_workers);
...
async function main() {
    try {
        ...
        const contextMsg = buildContextMessage(sessionId, session, taskCount, evidenceCount, sessionFile);
        const output = {
            hookSpecificOutput: {
                hookEventName: 'UserPromptSubmit',
...
//# sourceMappingURL=session-context-hook.js.map