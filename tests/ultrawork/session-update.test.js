...
describe('session-update.js', () => {
  ...
  describe('phase transition validation', () => {
    ...
    test('should allow valid transition VERIFICATION to DOCUMENTATION after --verifier-passed set', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION'
      });

      // Two-step: set flag first, then transition
      await runScript(SCRIPT_PATH, [
        '--session', session.sessionId,
        '--verifier-passed'
      ]);
  ...
  describe('DOCUMENTATION phase support', () => {
    test('should accept DOCUMENTATION as a valid phase', async () => {
      // Move to EXECUTION first, then VERIFICATION, then set verifier-passed, then DOCUMENTATION
      await runScript(SCRIPT_PATH, ['--session', session.sessionId, '--phase', 'EXECUTION']);
      await runScript(SCRIPT_PATH, ['--session', session.sessionId, '--phase', 'VERIFICATION']);
      await runScript(SCRIPT_PATH, ['--session', session.sessionId, '--verifier-passed']);
    ...
    test('should normalize DOCUMENTATION phase input', async () => {
      ...
      // VERIFICATION → DOCUMENTATION with lowercase phase name
      const result = await runScript(SCRIPT_PATH, [
        '--session', session.sessionId,
        '--phase', 'documentation'
      ]);
  ...
  describe('COMPLETE gate with documenter_completed', () => {
    ...
    test('should reject COMPLETE with --documenter-completed in same call (two-step required)', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'DOCUMENTATION',
        verifier_passed: true
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain('cannot be combined with --phase');
    });
  });
  ...
  describe('Phase transition guardrails', () => {
    ...
    test('should allow VERIFICATION → DOCUMENTATION after --verifier-passed set separately', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION'
      });

      // Two-step: set flag first
      await runScript(SCRIPT_PATH, [
        '--session', session.sessionId,
        '--verifier-passed'
      ]);
  ...
  describe('flag+phase combo rejection (two-step validation)', () => {
    test('should reject --verifier-passed combined with --phase', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION'
      });
    ...
    test('should reject --documenter-completed combined with --phase', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'DOCUMENTATION',
        verifier_passed: true
  ...
  describe('--verifier-passed prerequisite: all non-verify tasks resolved', () => {
    test('should reject --verifier-passed when non-verify tasks are unresolved', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION'
      });

      // Create tasks: one resolved, one open
      ...
      // verifier_passed should remain false
      const updated = readSession(session.sessionId);
      expect(updated.verifier_passed).toBe(false);
    });

    test('should allow --verifier-passed when all non-verify tasks are resolved', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION'
      });

      // Create resolved tasks plus verify task (non-resolved is OK for verify)
      createMockTask(session.sessionId, '1', { status: 'resolved' });
      createMockTask(session.sessionId, '2', { status: 'resolved' });
      createMockTask(session.sessionId, 'verify', { status: 'open' });
    ...
    test('should treat malformed task JSON as unresolved (fail-closed)', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION'
      });

      // Create a resolved task and a malformed task file
      createMockTask(session.sessionId, '1', { status: 'resolved' });
      const tasksDir = path.join(session.sessionDir, 'tasks');
      fs.writeFileSync(path.join(tasksDir, '2.json'), '{malformed json!!!', 'utf-8');
  ...
  describe('--documenter-completed prerequisite: verifier_passed required', () => {
    test('should reject --documenter-completed without verifier_passed', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'DOCUMENTATION',
        verifier_passed: false
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain('verifier_passed');

      // documenter_completed should remain false/undefined
      const updated = readSession(session.sessionId);
      expect(updated.documenter_completed).not.toBe(true);
    });

    test('should allow --documenter-completed with verifier_passed set', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'DOCUMENTATION',
        verifier_passed: true
  ...
  describe('documenter_completed reset on EXECUTION transition', () => {
    test('should reset documenter_completed when transitioning to EXECUTION', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION'
      });

      // Manually set documenter_completed in session file
      const sessionData = readSession(session.sessionId);
      sessionData.documenter_completed = true;
      fs.writeFileSync(session.sessionFile, JSON.stringify(sessionData, null, 2), 'utf-8');

      // Ralph loop: VERIFICATION -> EXECUTION
      const result = await runScript(SCRIPT_PATH, [
        '--session', session.sessionId,
        '--phase', 'EXECUTION'
      ]);
      ...
      expect(updated.phase).toBe('EXECUTION');
      expect(updated.verifier_passed).toBe(false);
      expect(updated.documenter_completed).toBe(false);
    });
  });

  describe('validation inside file lock (TOCTOU fix)', () => {
    test('should reject DOCUMENTATION gate check inside lock (no stale reads)', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'VERIFICATION',
        verifier_passed: false
      ...
      // Attempt to transition without verifier_passed
      const result = await runScript(SCRIPT_PATH, [
        '--session', session.sessionId,
        '--phase', 'DOCUMENTATION'
      ]);
    ...
    test('should reject COMPLETE gate check inside lock for missing verifier_passed', async () => {
      session.cleanup();
      session = createMockSession('test-session-update', {
        phase: 'DOCUMENTATION',
        verifier_passed: false
